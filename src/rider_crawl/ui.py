...
def coerce_settings(values: dict[str, Any]) -> UiSettings:
    interval_minutes = _positive_int(values["interval_minutes"], "실행 간격")
...
class RiderBotUi:
    ...
    def _build(self) -> None:
        ...
        title = ttk.Label(outer, text="쿠팡이츠 실시간 실적봇", font=("", 18, "bold"))
        title.grid(row=0, column=0, sticky="w")

        subtitle = ttk.Label(
            outer,
            text="로그인된 쿠팡이츠 페이지를 읽고 카카오톡 단체방에 텍스트 실적을 보냅니다.",
        )
    ...
    def _build_settings(self, parent: ttk.Frame) -> ttk.Frame:
        ...
        rows = [
            ("실적 URL", "performance_url"),
            ("피크 대시보드 URL", "peak_dashboard_url"),
            ("CDP 주소", "cdp_url"),
            ("앱 전용 브라우저 프로필 경로", "browser_user_data_dir"),
            ("로그 경로", "log_dir"),
            ("카카오톡 채팅방명", "kakao_chat_name"),
            ("실행 간격(분)", "interval_minutes"),
    ...
    def _build_runtime(self, parent: ttk.Frame) -> ttk.Frame:
        ...
        ttk.Label(
            checklist,
            text=(
                "1. 쿠팡이츠 링크는 로그인된 상태로 열려 있어야 합니다.\n"
                ...
                "6. 시작 버튼을 누르면 즉시 1회 실행 후 설정한 간격으로 반복됩니다."
            ),
            justify="left",
