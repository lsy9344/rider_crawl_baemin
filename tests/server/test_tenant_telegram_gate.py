...
def test_secret_resolver_keeps_tenant_secrets_inside_running_loop() -> None:
    async def _run() -> list[str]:
        provider = _FakeProvider({"t1": _cfg("t1", secret="TENANT_A")})
        resolve = _default_resolve_telegram_secret(_settings(), provider)
        maybe_secrets = resolve()
        assert isawaitable(maybe_secrets)
        return await maybe_secrets

    assert asyncio.run(_run()) == ["TENANT_A"]
