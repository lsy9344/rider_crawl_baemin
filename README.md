...
배민 `배달현황` 또는 쿠팡이츠 실적 페이지를 읽어 텔레그램 또는 카카오톡으로 보낼 텍스트 메시지를 생성하는 Python 봇입니다.
...
- 실적/배달현황 URL(주 URL): 배민 기본값 `https://deliverycenter.baemin.com/delivery/history?page=0&size=20&orderName=name&orderBy=asc&name=&userId=&phoneNumber=&riderStatus=`, 쿠팡이츠는 `https://partner.coupangeats.com/page/rider-performance`. 이 URL이 비어 있으면 탭은 비활성으로 간주합니다.
...
| 변수 | 설명 |
| --- | --- |
| `BAEMIN_DELIVERY_HISTORY_URL` | 배민 배달현황 주 URL(미설정 시 기본값) |
...
- 배민 탭: 배달현황 페이지의 `table`, `th`, `td`를 읽고 헤더 텍스트 기준으로 값을 매핑합니다. `합계` 행은 요약값으로 사용하고, 일반 라이더 행은 이름과 운행상태 기준으로 수행 중인 인원을 계산하는 데 사용합니다.
