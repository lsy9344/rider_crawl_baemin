...
Docker Compose 실행 전 DB 비밀번호를 환경 변수로 주입합니다. 값은 저장소에 커밋하지 마세요.
기본 compose는 `127.0.0.1:8000`에만 바인딩합니다. EC2에서 외부 공개가 필요하면 보안그룹
...
compose는 `db` healthcheck 후 `migrate` one-shot으로 Alembic migration을 적용하고, 성공 뒤 `backend-api`와 `scheduler`를 시작합니다. 외부 DB를 쓰는 배포에서는 compose의 `DATABASE_URL` environment를 운영 secret 주입 값으로 override하세요.
...
- 쿠팡이츠 로그인이 만료되면 기본적으로는 자동 로그인이나 2차 인증 처리를 하지 않고 해당 크롤링 탭의 반복 실행을 중지합니다. Chrome에서 다시 로그인한 뒤 `peak-dashboard` 페이지를 로그인된 상태로 열어두고 `시작`을 다시 누르세요.
