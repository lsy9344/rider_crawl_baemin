...
현재 코드에는 `Customer`, `Tenant`, `Account` 같은 도메인 객체가 없다. 대신 UI 탭 하나가 고객 또는 계정 하나처럼 동작한다.
...
- 저장은 단순 `write_text()`다.
- atomic write가 아니다.
- 전원 종료 또는 프로세스 종료가 파일 쓰기 중 발생하면 일부 파일이 깨질 가능성이 있다.
- 쿠팡 비밀번호, 텔레그램 토큰, Gmail token 경로 같은 민감 설정도 같은 JSON에 저장된다.
