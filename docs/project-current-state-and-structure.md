...
민감값은 기록하지 않는다. 예를 들어 텔레그램 토큰, 채팅방 ID, 쿠팡 비밀번호, Gmail OAuth 토큰 값은 이 문서에 쓰지 않는다.
...
| Gmail token | 쿠팡 이메일 인증번호를 읽기 위해 Gmail API 접근 권한을 저장한 OAuth 파일 |
| 자동복구 | 로그인 만료 후 프로그램이 다시 로그인 또는 인증을 시도하는 기능 |

## 0.3 현재 중요한 전제

- 배민은 휴대폰번호 인증만 존재한다고 본다. 따라서 완전 자동 로그인이 어렵다.
- 쿠팡이츠는 이메일 인증이 존재한다. 그래서 Gmail API를 사용하면 자동 인증 가능성이 있다.
- 단, 쿠팡 고객마다 인증메일을 받는 Gmail이 다를 수 있다. 이 경우 Gmail token도 고객별로 분리해야 한다.
- 현재 코드는 중앙 서버가 아니라 로컬 PC 앱이다.
...
3. 쿠팡 Gmail 인증은 고객별 Gmail token, 보안 저장소, 재승인 흐름을 어떻게 설계해야 하는가?
...
7. 비밀번호, Gmail token, 텔레그램 token 같은 비밀값은 어떻게 안전하게 저장해야 하는가?
...
현재 프로젝트는 중앙 서버형 SaaS가 아니라, 고객 또는 운영자 PC에서 실행하는 Python 기반 데스크톱 자동화 프로그램이다. Tkinter UI에서 `크롤링1`부터 `크롤링9`까지 탭을 만들고, 각 탭이 하나의 배민 또는 쿠팡이츠 계정 세션을 담당한다. 각 탭은 별도 Chrome 프로필과 별도 CDP 포트에 연결해 로그인된 웹 화면을 읽고, 텔레그램 또는 카카오톡으로 실적 메시지를 보낸다.

현재 구조는 소수 계정 운영에는 맞지만, 100명 또는 1000명 고객을 판매용으로 운영하기에는 탭 기반 UI, 로컬 Chrome 세션 관리, 평문 비밀값 저장, 중앙 모니터링 부재 때문에 한계가 크다.
...
4. Chrome 준비 버튼으로 별도 Chrome 프로필을 연다.
5. 운영자가 쿠팡에 로그인한다.
6. 선택적으로 쿠팡 아이디/비밀번호와 Gmail token 경로를 등록한다.
7. 프로그램이 쿠팡 peak-dashboard 화면을 읽는다.
8. 로그인 만료 시 Gmail 자동 2FA가 켜져 있으면 이메일 인증번호를 읽어 복구를 시도한다.
...
- 현재 프로그램은 중앙 서버가 아니다.
- 현재 프로그램은 고객별 계정/상태를 DB로 관리하지 않는다.
- 현재 프로그램의 `탭`은 사실상 고객/계정 역할을 대신하는 임시 모델이다.
- 배민 휴대폰 인증은 현재 자동 처리하지 않는다.
- 쿠팡 Gmail 인증은 자동화 가능성이 있지만 고객별 Gmail token 분리가 필요하다.
...
| `src/rider_server/` | Epic 2에서 추가된 플랫폼 중립 도메인/서비스 계층(`domain`·`services`·`migration`). 순수·의존성 0이며 아직 런타임 미배선(DB/ORM·API는 Epic 5) |
| `tests/` | pytest 기반 단위 테스트와 일부 UI helper 테스트 |
| `docs/` | 설계 문서, 구현 계획, 운영 문서 |
| `runtime/` | 실행 중 생성되는 로컬 상태 파일 |
| `logs/` | 실행 오류 로그 등 로컬 로그 |
| `secrets/` | Google/Gmail OAuth 파일 위치. 실제 JSON 파일은 Git 제외 |
| `scripts/` | 운영 보조 스크립트. 현재 Gmail OAuth token 생성 스크립트 포함 |
...
| `gmail_2fa_query` | Gmail 인증메일 검색식 |
| `gmail_credentials_path` | Gmail OAuth client 파일 경로 |
| `gmail_token_path` | Gmail OAuth token 파일 경로 |
...
5. Gmail 자동 2FA가 꺼져 있으면 탭 실행을 중지한다.
6. Gmail 자동 2FA가 켜져 있으면 한 번 복구를 시도한다.
...
4. 이메일 인증 방식을 클릭한다.
5. 인증번호 발송 버튼을 클릭한다.
6. 발송 직전 시각을 기준으로 Gmail을 polling한다.
7. Gmail에서 요청 시각 이후 도착한 최신 인증메일을 찾는다.
8. 메일 본문에서 인증번호를 추출한다.
9. 쿠팡 인증번호 입력칸에 입력한다.
10. 제출 버튼을 누른다.

Gmail API 구현은 `auth/gmail.py`에 있다.
...
- 인증번호, OAuth token 값은 예외 메시지나 로그에 넣지 않는다.
- token이 만료됐고 refresh token이 있으면 refresh 후 token 파일을 다시 저장한다.

여러 고객 Gmail 처리:

- OAuth client 파일 `credentials.gmail.json`은 공통 1개로 가능하다.
- Gmail token 파일은 Gmail 받은편지함마다 1개가 필요하다.
- 고객마다 Gmail 계정이 다르면 `token.<customer_id>.json`처럼 분리해야 한다.
- 여러 쿠팡 계정이 같은 Gmail 받은편지함으로 인증메일을 받으면, 동시 인증 요청 때 최신 메일을 잘못 집을 수 있다.
- 따라서 판매용 구조에서는 고객별 Gmail token과 고객별 Gmail 검색 조건을 명확히 분리해야 한다.
...
| `google-auth-oauthlib` | Gmail OAuth token 생성 |
...
- `secrets/google/*.json`은 Git에 올리지 않는다.
- 고객별 Gmail token 파일도 배포/백업/보호 정책이 필요하다.
...
- 쿠팡 Gmail 2FA 자동복구의 기본 구조가 이미 있다.
- 인증번호와 OAuth token 값이 로그에 직접 남지 않도록 주의한 코드가 있다.
...
### 26.5 쿠팡 Gmail은 고객별 token 분리가 필수다

쿠팡은 이메일 인증이므로 Gmail API 자동화가 가능하다. 하지만 고객마다 쓰는 Gmail이 다르면 token 파일도 반드시 분리해야 한다.
...
현재 코드를 전부 버릴 필요는 없다. 크롤러, parser, message renderer, Gmail 2FA 일부는 재사용할 수 있다. 다만 운영 구조는 바꿔야 한다.
...
3. 고객 Gmail OAuth를 1회 승인한다.
4. token은 고객별로 저장한다. 예: `token.<customer_id>.json`
5. 에이전트가 쿠팡 로그인 페이지를 연다.
6. 필요하면 아이디/비밀번호를 입력한다.
7. 이메일 인증번호를 Gmail API로 읽는다.
8. 인증번호를 입력한다.
...
- `UiSettings`에 `customer_id`, `customer_name` 추가
- `state_subdir`를 `crawlingN`이 아니라 `customer_id` 기준으로 변경
- `baemin_center_name`을 플랫폼 중립 이름으로 정리
- `coupang_eats_url`을 `performance_url` 또는 `primary_url`로 정리
- README의 쿠팡 설명을 현재 코드와 맞게 수정
- 저장 파일 write를 atomic write로 변경
- 로그 rotation 추가
...
- UI와 크롤링 실행 엔진 분리
- 크롤링 엔진은 백그라운드 서비스처럼 동작
- 설정은 로컬 JSON이 아니라 중앙 서버에서 받아오도록 준비
- 에이전트 등록 코드 방식 추가
- 앱 버전 보고 추가
...
4. 쿠팡은 Gmail API 기반 자동복구를 살리되, Gmail token을 고객별/메일함별로 분리한다.
...
- 쿠팡이츠는 이메일 인증이 있으므로 Gmail API 기반 자동 인증을 고려할 수 있습니다.
- 쿠팡 고객마다 인증메일을 받는 Gmail 계정이 다를 수 있으므로 Gmail OAuth token은 고객별/메일함별로 분리해야 합니다.
- 현재 프로그램은 중앙 서버가 아니라 고객/운영자 PC에서 실행되는 Python Tkinter 앱입니다.
...
6. 쿠팡 Gmail 2FA 자동화와 고객별 token 관리 방식
7. 비밀번호, Gmail token, 텔레그램 token 보안 저장 방식
...
| 쿠팡 인증 이해 | Gmail token을 고객별로 분리하고 재승인 흐름 포함 | 모든 고객이 같은 Gmail token을 공유한다고 가정 |
...
이 프로젝트는 이미 작동하는 로컬 자동화 도구의 기반을 갖고 있다. 특히 플랫폼별 parser, 메시지 생성, 텔레그램 전송, 쿠팡 Gmail 2FA는 판매용 시스템에서도 일부 재사용할 수 있다.
...
배민은 “자동 로그인”이 아니라 “인증 필요 감지와 사용자 안내”가 핵심이다. 쿠팡은 “Gmail API 자동 인증과 고객별 token 관리”가 핵심이다. 이 차이를 정확히 반영한 개선안만 현실성이 있다.
