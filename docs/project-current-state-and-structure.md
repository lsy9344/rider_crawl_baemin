...
- 단, 쿠팡 고객마다 인증메일을 받는 메일함이 다를 수 있다. 이 경우 인증 이메일 주소와 앱 비밀번호도 고객별로 분리해야 한다.
...
- `LOG_DIR`
- `STATE_SUBDIR`
- 쿠팡 이메일 자동복구는 현행 UI/Agent 설정의 인증 이메일 주소와 앱 비밀번호를 사용한다.
...
- 현재 웹 관리자 방식에서는 앱 비밀번호를 DB에 저장하고, Agent job payload로 전달해 로그인 복구에 사용한다. 로그/job 결과에는 값을 남기지 않는다.

여러 고객 메일함 처리:

- 인증 이메일 주소와 앱 비밀번호는 고객/메일함별로 분리한다.
...
쿠팡은 이메일 인증이므로 Gmail/Naver IMAP 자동화가 가능하다. 하지만 고객마다 쓰는 메일함이 다르면 인증 이메일 주소와 앱 비밀번호도 반드시 분리해야 한다.
...
- 쿠팡 고객마다 인증메일을 받는 메일함이 다를 수 있으므로 인증 이메일 주소와 앱 비밀번호는 고객별/메일함별로 분리해야 합니다.
