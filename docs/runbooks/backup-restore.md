...
- **현재 저비용 모드**: Terraform 구성은 단일 EC2 루트 EBS를 DLM으로 매일 1회 스냅샷하고
  7일 보존한다. PostgreSQL은 RDS가 아니라 Docker Compose 로컬 DB다.
- **PITR(Point-In-Time Recovery)**: 현재 구성에는 WAL 아카이빙 기반 PITR이 없다. 최대 RPO는
  마지막 성공 EBS 스냅샷 이후 변경분, 즉 최악의 경우 약 24시간이다.
- **운영 고도화 목표**: 상용 다중테넌트/강한 RPO가 필요해지면 RDS Multi-AZ 또는 별도 DB와
  WAL 아카이빙/PITR을 도입한다.
- **보존 기간**: 자동 스냅샷 **최소 7일** 보존. 규제/사고 분석 요구가 늘면 보존 기간을 연장한다.
...
2. 마이그레이션 **직전 수동 EBS 스냅샷**을 1건 만든다(라벨: `pre-<revision>` 예 `pre-0019`).
...
`0015_delivery_outbox`는 legacy `messages.text`를 기존 redacted preview로 backfill한다. 이 값이 실전송 본문으로 나가지 않게, migration은 기존 sendable delivery row(`RETRYING`/`FAILED`)를 먼저 `HELD`로 전환한다. 적용 후 운영자는 `HELD` row를 검토해 원문 복원이 가능한 건만 재생성하거나 폐기한다.
