...
작성일: 2026-06-19  
반영 상태: 1단계 반영완료(현재 반영 범위 기준). 단계별 자동 전달 wizard·완료 화면·세팅 상태 모델/타임스탬프 저장은 각 절의 "후속 범위"로 남았다.  
...
- 반영됨: `/admin` DB 실패 안내, `새 고객 세팅 시작` 안내 섹션 + 단계 순서, 기본/고급 설정 분리, 운영자 친화 용어, 타이포그래피/접근성 보강, 실발송 ON UI 비활성 + 서버 fail-closed 거부. 서버 게이트는 `sending_enabled` 의 **OFF→ON 전이**를 거부한다(테스트 통과 상태 모델이 아직 없어 임시 게이트). 이미 켜진 고객의 다른 필드 편집 시 폼이 현재 값 `sending_enabled=True` 를 그대로 재전송하는 no-op 은 거부하지 않고, OFF 로 끄는 것은 항상 허용한다(`tenant_service.py`).
- 후속(미반영): 단계 간 결과 자동 전달 full wizard, `세팅 필요/로그인 필요/.../운영 중` 상태 모델, `수집/전송 테스트 통과 시각`·`실발송 변경자/시각` 저장, 테스트 통과를 보고 여는 동적 실발송 게이트. 현재 게이트는 테스트 통과 상태를 보고 여는 구조가 아니라 OFF→ON 전이를 일괄 차단하는 임시 방식이다.
...
- 관리 탭 상단에 `새 고객 세팅 시작` 섹션을 추가한다.
- 섹션은 새 고객 생성 폼과 세팅 순서 안내를 제공한다.
...
- 기존 HTMX select/options 경로를 사용해 raw id 복사 입력을 줄인다.
- 단계별 자동 전달과 완료 화면은 아직 구현하지 않는다.
- 마지막 단계 전에는 실발송 ON을 켤 수 없게 하고, 수집 테스트와 전송 테스트 미완료 상태를 강하게 안내한다.

검증:

- 템플릿 테스트에서 `새 고객 세팅 시작` 섹션과 단계 제목이 렌더되는지 확인한다.
- 옵션 select가 기존 HTMX 옵션 경로를 계속 사용하는지 확인한다.
- 테스트 완료 전 실발송 ON 단계가 비활성이고 강한 안내 상태인지 확인한다.

후속 범위:

- 이전 단계 결과를 다음 단계에 자동 전달하는 full wizard.
- `수집 테스트 통과`, `전송 테스트 통과`, `실발송 OFF/ON`을 보여주는 완료 화면.
...
- `새 고객 세팅 시작` 섹션에 `수집 테스트: 미완료`, `전송 테스트: 미완료` 안내를 표시한다.
- 텔레그램 설정의 `실제 메시지 보내기` ON 옵션은 비활성화한다.
- 서버도 `sending_enabled` 의 OFF→ON 전이를 거부해 테스트 완료 전 실발송 켜기를 막는다(이미 켜진 고객의 no-op 재전송은 허용, OFF 로 끄기는 항상 허용).

검증:

- 안내 문구와 비활성 ON 옵션이 렌더되는지 확인한다.
- crafted request로 OFF 상태에서 `sending_enabled=True`(OFF→ON 전이)가 들어오면 서버가 거부하는지 확인한다.

후속 범위:

- 고객 또는 업체별 설정 상태 badge.
- `세팅 필요`, `로그인 필요`, `수집 테스트 필요`, `전송 테스트 필요`, `실발송 대기`, `운영 중` 상태 모델.
- `수집 테스트 통과 시각`, `전송 테스트 통과 시각`, `실발송 변경자`, `실발송 변경 시각` 저장.
...
- 실발송 전 수집 테스트와 전송 테스트가 필요하다는 안내가 보인다.
- 테스트 완료 전 실발송 ON이 UI와 서버에서 차단된다.
