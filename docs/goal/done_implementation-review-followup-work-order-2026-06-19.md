...
완료 상태: 작업지시서의 후속 작업(Task 1~7)을 코드/테스트에 반영 완료했다.

> 문서 정합성 주의(2026-06-21 갱신): 아래 "검토 요약"과 각 Task의 "문제" 절은 **작성 당시
> (반영 전) 진단**이다. 현재 코드 기준으로는 7개 Task가 모두 반영됐다(예: Telegram dispatch
> 는 실제 발송 경로로 이어지고, legacy migration 은 SecretRef 핸들만 매핑하며, dashboard
> pagination 은 DB 단계에서 limit/offset 을 적용하고, crawl timeout 은 subprocess boundary +
> 프로세스 트리 kill 로 처리한다). 본문 체크박스(`- [ ]`)는 작성 시 양식이며, 실제 반영 여부는
> 각 Task 의 "검증" 명령과 현재 코드/테스트를 기준으로 본다.

작성일: 2026-06-19
대상: `docs/goal/done_*` 검토결과/작업지시서 반영 후 현재 작업트리
목적: 완료 표시된 작업 중 실제 구현이 부족하거나 수락 기준을 만족하지 못한 항목을 보완한다.

## 검토 요약(작성 당시 진단 — 현재는 모두 반영됨)
