...
**Event/Job Patterns:**
- job type: UPPER_SNAKE. 구현(Epic 4)·heartbeat capabilities·data-api-contract의
  정본 Agent job type은 `CRAWL_BAEMIN`, `CRAWL_COUPANG`, `AUTH_CHECK`,
  `OPEN_AUTH_BROWSER`, `KAKAO_SEND`, `CAPTURE_DIAGNOSTIC`이다(초안의 `CRAWL`/`RENDER`/
  `DISPATCH_TELEGRAM`/`BAEMIN_AUTH_OPEN`은 구표기 — 배민 인증 job은 `AUTH_CHECK`/
  `OPEN_AUTH_BROWSER`로 분리됨). 상태 전이는 정의된 set만 허용.
